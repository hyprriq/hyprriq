      ...
      <div className="mx-auto max-w-3xl space-y-5">
          ...
          {plan ? (
            <>
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <StripePortalButton className="rounded-lg border border-line bg-surface px-4 py-2 text-[13px] font-semibold text-ink-2 hover:bg-subtle">
                  Manage subscription →
                </StripePortalButton>
              </div>
              <div className="mt-4 grid grid-cols-3 gap-3">
                <div className="rounded-lg border border-line bg-base p-3">
                  <div className="text-[11px] uppercase tracking-wide text-muted">Credits Left</div>
                  <div className="mt-0.5 font-display text-2xl font-extrabold text-ink">{client.credits_available}</div>
                  <div className="text-[11px] text-muted">of {total} this cycle</div>
                </div>
                <div className="rounded-lg border border-line bg-base p-3">
                  <div className="text-[11px] uppercase tracking-wide text-muted">Rollover</div>
                  <div className="mt-0.5 font-display text-2xl font-extrabold text-ink">{rollover}</div>
                  <div className="text-[11px] text-muted">max carry-forward</div>
                </div>
                <div className="rounded-lg border border-line bg-base p-3">
                  <div className="text-[11px] uppercase tracking-wide text-muted">Renews</div>
                  <div className="mt-0.5 font-display text-base font-extrabold text-ink">{fmtDate(client.renewal_date)}</div>
                  <div className="text-[11px] text-muted">{renewDays !== null ? `${renewDays} days` : "—"}</div>
                </div>
              </div>
              <div className="mt-4">
                <div className="mb-1.5 text-[11px] text-muted">{client.credits_available} of {total} credits remaining this cycle</div>
        ...
        <Card title="Payment Method">
          <div className="flex flex-wrap items-center gap-3">
            <span className="text-[13px] text-ink-2">Manage your card and billing details securely in Stripe.</span>
            <StripePortalButton className="ml-auto rounded-lg bg-brand px-4 py-2 text-[13px] font-semibold text-white hover:bg-brand-hover">
              Manage in Stripe →
            </StripePortalButton>
          </div>
        </Card>
        ...
        <Card title="Invoice History">
          {invoices.length === 0 ? (
            ...
            <div className="divide-y divide-line">
              <div className="grid grid-cols-[80px_1fr_100px_56px] gap-3 pb-2 text-[10.5px] font-bold uppercase tracking-wide text-muted">
                <span>Date</span><span>Description</span><span>Amount</span><span></span>
              </div>
              {invoices.map((inv) => (
                <div key={inv.id} className="grid grid-cols-[80px_1fr_100px_56px] items-center gap-3 py-2.5 text-[13px]">
                  ...
                  {inv.pdf ? (
                    <a href={inv.pdf} target="_blank" rel="noopener noreferrer" className="text-[12px] font-semibold text-brand hover:underline">
                      PDF
                    </a>
                  ) : (
