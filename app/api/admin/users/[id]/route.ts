...
import { getOperator, canManageUsers, CAPABILITIES, type Capability } from "@/lib/auth/permissions";

// PATCH — edit a sub-user's capabilities or disable them. Super-admin only; never your own row;
// never a super_admin row (seeded rows are managed by the founder in SQL, not the API).
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (Array.isArray(body.capabilities)) {
    fields.capabilities = body.capabilities.filter((c): c is Capability => (CAPABILITIES as readonly string[]).includes(c));
  }
