...
**Status:** 🟡 **REVIEWED (founder, 2026-07-09) — ALL THREE SIGN-OFFS SIGNED; OQ-A/C/D RULED; OQ-B HELD pending founder ruling on the exact client copy (records inline below). BUILD NOT YET AUTHORIZED — authorization lands when OQ-B is ruled; then TDD per the task list, STOP before founder ATs.** Gate rhythm unchanged: TDD build (commit per task, tsc+eslint+vitest+build) → push staging → founder personally runs ATs → SB-1 frozen.
...
- **OQ-B — 🔴 HELD. Founder will rule on the exact copy before build.** **The bar set by the founder: the note must state OUR limitation, never a conclusion about the supplier.** Bad: "your supplier's website could not be confirmed" (reads supplier-is-suspect). Good: "We could not independently verify this domain from public sources in this pass" (a statement about our evidence). The spec's original draft ("could not independently confirm the operating business behind the website") FAILS this bar — "operating business behind the website" edges toward a supplier conclusion — and its "received manual identity review" sentence publishes process noise (the same reasoning that rejected OQ-A's alternative). Candidate replacements presented to the founder in the review thread; the ruled copy gets recorded HERE and becomes Task 3's exact expected string.
