...
**Status:** ✅ **SB-1 FROZEN (founder-declared, 2026-07-10) — ALL FIVE ATs PASSED, NOTHING DEFERRED-LIVE.** AT-1 ✅ (record below) · AT-1b ✅ (record below) · **AT-2 ✅** (streesutra.com, registrar-parked: 15 sources of homonym noise, model honestly found no operating business → website_dead with the ruled OQ-B copy verbatim — the TARGET branch, not the zero-source shortcut; affirmative-only rules holding) · **AT-3 ✅** (dead-key on AWI-2607-018: `resolution_research` website record `llm_failed: true`, `identity_discrepancy` NULL — no client note anywhere, truthful admin note, **the 429-masquerade is dead**; Tracks 1–2 also llm_failed = expected H2 behavior) · **AT-4 ✅** (rejudge determinism on the frozen delivered fixture) · **AT-5 ✅** (unit-locked, green). **Bonus closures:** SO-2 proven live in BOTH directions (llm_failed false on AT-1/AT-2, true on AT-3); SO-3's ruled copy shipped verbatim three times across two note classes. **Corpus correction (nike) completed + verified:** event stripped via the documented escape hatch, rollup deleted, trigger re-armed, audit_log written, brand_intelligence back to 7 rows. Build record: all SOs signed + OQs ruled inline; built Fable TDD commits 2dc7479..4454a57; 480/480 tests; NO migration; PIPELINE_VERSION 1.2.0. Frozen core untouched (`identityResolver.ts` byte-identical). **Next gate: SB-2** (comparator; spec `2026-07-10-sb2-identity-comparator-gate.md`).
...
**AT-1 ✅ PASSED (founder, 2026-07-10, corrected fixture AWI-2607-026 "Bulk Electronics Wholesale Vendor" + tdsynnex.com):** `resolved_from_website` / `resolved_domain=tdsynnex.com` / `identity_unconfirmed=false` / `resolved_name="TD SYNNEX Corporation"` / `input_consistency=low` / kind `name_website_mismatch` / audit winner `tdsynnex.com` (was null/0). Both research calls resolved (14/15 sources, `llm_failed:false`). **The TLD-derivation false negative is DEAD. 🎁 H4's logged limitation CLOSED — first live resolved-to-different-entity demo (recorded on the tracker's H4 line).** The live `name_website_mismatch` note ("…the website provided belongs to TD SYNNEX Corporation. This report's analysis is based on…") passes the OQ-B bar cleanly and is recorded as **SB-2's reference output** — where same-entity cases should route instead of the false "different businesses" note.

**AT-1b — wrong-domain behavior check (globaldist.com, live) — the fix must NOT falsely resolve it.**
**FIXTURE RULED (founder, 2026-07-09): AWI-2607-018 (`086655bb-1e0d-4818-9df7-2343e650ea0d`, vendor_name "Bosch", website globaldist.com)** — a STRONGER test than planned: the hardest possible wrong-domain pressure (globally-famous brand name + a live domain owned by an unrelated third party, openborder). If the resolver still refuses to confer "Bosch" identity from globaldist.com, the by-construction guarantee holds under maximum pressure. Expected: outcome (b) `multiple_entities` — and HERE the escalation is TRUTHFUL (Bosch and openborder genuinely are different businesses), a clean contrast with the TD Synexx first run where the same code path produced a FALSE escalation (recorded in SB-2's rationale — exactly why domain-first comparison matters). One behavioral caveat: if this case's `brands_submitted` includes "Bosch", rule 3 short-circuits the name research → outcome is `name_is_brand` resolving to the openborder entity with the note naming it — ALSO a PASS and also truthful (the client is told whose site it is). Either way, FAIL remains: `resolved_name = "Bosch"`.
**AT-1b ✅ PASSED (founder, 2026-07-10): outcome (c) `website_dead`** — `unresolved` / `resolved_domain=null` / `unconfirmed=true`; the FAIL condition did not fire despite maximum pressure (globally-famous name + third-party live domain). `resolution_research` shows ONE record (website, 15 sources) — the name call never ran because **openborder did not earn dominance on globaldist.com** (branch 2b never reached; dominance still has to be EARNED post-fix — the two-sided guarantee demonstrated live). **The OQ-B website_dead copy shipped live verbatim — SO-3 proven live.** RIDER LOGGED (tracker): on an unresolved record, `resolved_name` carries the ENTERED name ("Bosch") while nothing resolved — semantically confusing (should be null; the entered name already lives in `entered_name`/`original_input`); made the pass/fail read ambiguous at first glance. Natural home: SB-2. Post-SB-1, ANY of these outcomes is a PASS: (a) resolves to the domain's ACTUAL occupant (the openborder entity) with a `name_website_mismatch` note naming THAT entity — the client is told whose site this is; (b) `multiple_entities` escalation if the entered name also resolves its own entity; (c) `website_dead` escalation if the occupant doesn't earn dominance this pass. **FAIL = any outcome where `resolved_name` equals the entered supplier name** — identity conferred by someone else's domain would be the wrong-but-confirmed regression. Mechanically impossible by construction (`resolved_name` derives ONLY from the LLM-discovered entity FOR the domain; anchor identity is `canonicalDomain` equality, which carries no name semantics) and unit-locked both ways (`track05.test.ts` wrong-domain locks, added with this correction).
