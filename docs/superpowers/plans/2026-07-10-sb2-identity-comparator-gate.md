...
**Status:** 🔴 **DRAFT — AWAITING FOUNDER REVIEW. NO CODE UNTIL: all four SIGN-OFFS signed + all four OQs ruled.** Gate rhythm: this spec → founder rules → TDD build (commit per task, tsc+eslint+vitest+build) → push staging → founder personally runs ATs → SB-2 frozen.
