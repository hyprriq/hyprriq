...
**Status:** 🟡 **REVIEWED (founder, 2026-07-10) — ALL FOUR SIGN-OFFS SIGNED (SO-2 with two conditions LOCKED AS TESTS; SO-3 pending OQ-B's exact string); OQ-A/C/D RULED; OQ-B HELD pending founder ruling on the copy. BUILD NOT AUTHORIZED until OQ-B is ruled.**
...
- **OQ-B — the replacement `multiple_entities` copy.** Post-SB-2 the note fires only on genuine two-entity findings, but it must still state OUR limitation, not insinuate misrepresentation. **Proposed:** *"Identity clarification: In this pass, we could not confirm from public sources that the supplier name "{entered}" and the website you provided refer to the same business — our research found separate results for each. This reflects a limit of our verification in this investigation, not a finding about the supplier. Please contact us to confirm your intended supplier before relying on the findings."* Founder rules exact wording (ships in client reports).
