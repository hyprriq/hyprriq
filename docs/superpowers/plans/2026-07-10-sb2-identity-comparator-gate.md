...
**Status:** ✅ **SB-2 FROZEN (founder-declared, 2026-07-10) — ALL ATs PASSED; only OQ-D's live two-entity half deferred (sentinel in place).**
**AT-1 ✅** (AWI-2606-012 — the case that exposed SB-2, now its proof): `resolved_from_website`, kind `name_website_mismatch` (was the false `multiple_entities`), unconfirmed=false, resolved_domain=tdsynnex.com; `resolution_research` shows BOTH calls with winner "tdsynnex.com", score 4, matched_by ["name_match","self_identifies"] — website side and name side (researching the typo) landed on the SAME domain; the old comparator saw two name strings, the new one sees identical winning domains. **Same-entity false ambiguity DEAD.** Reference-output note verbatim. Also re-proves SB-1's TLD fix live (name_match earned on a sub-12-char label). **AT-2 ✅ conscience lock HELD** (AWI-2607-018 Bosch/globaldist: unresolved/website_dead, no identity conferred; resolved_name="Bosch" = the OQ-C research-subject semantics as ruled; name call never ran — dominance still gates the comparator, the narrowing sits BEHIND it). **SO-4 PAYOFF, proven both directions:** the carried audit exposed WHY globaldist escalates — `{score: 2, winner: null, runner_up: "gdl-logistics.com", runner_up_score: 2, matched_by: ["self_identifies"]}`: a 2-vs-2 tie between two candidates, neither dominant — **escalations are now diagnosable instead of opaque.** **AT-3 ✅** unit-locked · **AT-4 ✅** rejudge determinism · **AT-5 ✅** all SB-1 regression locks green. OQ-B copy shipped verbatim a third time; reads correctly every time. Build record below stands; PIPELINE_VERSION 1.3.0; 493/493; NO migration; `identityResolver.ts` byte-identical. **Next gate: Track 3.**
