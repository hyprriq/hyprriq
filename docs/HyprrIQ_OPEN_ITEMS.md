...
> | **Acquisition grant** — recon done: `clients.referral_code` + `clients.referred_by` EXIST (attribution needs NO migration); the grants table needs ONE founder-run migration whose shape should come from `DEFERRED_acquisition_system_spec.md` | ⛔ BLOCKED on the spec (not reachable from this machine — founder pastes it) | ruled: 1 free full-report credit (all areas + category compliance), 30-day expiry, no commission yet, attribution captured at signup, "full assessment" framing |
