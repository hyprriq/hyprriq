...
> | **④ Fabricated verdict** (`report-view.tsx` `verdict ?? "verify_before_purchase"`) | 📋 REPORT-ONLY, by instruction | census + throw-design in the session deliverable — THREE render sites incl. the PDF pipeline (`renderReportPdf.ts:131`); verdict-adjacent, untouched |
