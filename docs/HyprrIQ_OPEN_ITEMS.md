...
> | **Retention sweep** — daily cron, ⛔ OFF until founder sets `RETENTION_SWEEP_ENABLED=1`: source documents past `delete_after` (storage removed → `deleted_at` stamped → audited; failed removal retries tomorrow, never a stamped row with a live object) + dormant one-time 24-month notice (once ever, by unique key) | ✅ SHIPPED, flag off | frozen Evidence Packs survive — findings/reports/rejudge unaffected |
