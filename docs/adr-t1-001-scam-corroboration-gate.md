...
| `registration_fabricated` | `government_registration` | `{government_record,registry}` · `high` | **Sibling — firewall UNCHANGED, prompt-mitigated** (founder decision 2026-07-03; see below) |
...
- **`registration_fabricated` — RESOLVED (founder decision 2026-07-03):** structurally the same collision, but its sources are **high-authority** (`government_record`/`registry`, `MIN_AUTHORITY="high"`), so a single authoritative record can legitimately establish registration fraud — a ≥2 corroboration gate would **over-reject** valid single-source findings (the opposite failure). **DECISION: no corroboration gate; firewall behavior preserved unchanged.** The correct lever is **prompt-level classification guidance** (Track 1 prompt, `commit` below): `registration_fabricated` is ONLY for AFFIRMATIVE fabrication (registry shows the registration nonexistent, forged, or contradicted by the official record); **not-found / cannot-verify / inactive-or-dissolved-but-real / different-parent-DBA-entity → UNKNOWN, never `registration_fabricated`.**
