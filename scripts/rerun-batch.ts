...
function preflight() {
  ...
  if (VALIDATION_VERSION !== "1.5.0") { // H7 (SO-2) — pin tracks the firewall version deliberately
    console.error(`STOP: this code is VALIDATION_VERSION "${VALIDATION_VERSION}", expected "1.5.0". The fix is NOT deployed here — aborting so we don't re-score under old logic.`);
    process.exit(1);
  }
