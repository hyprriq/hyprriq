        ...
        <div className="mt-3 flex flex-wrap items-end gap-3">
          <label className="text-[12px] text-muted">
            ...
            <select value={mode} onChange={(e) => setMode(e.target.value as "link" | "coupon")}
              className="mt-1 block rounded-lg border border-line bg-canvas px-2.5 py-2 text-[13px] text-ink">
                    ...
                    <tr key={g.id} className="border-b border-line/60 align-top">
                      ...
                      <td className="py-2.5 text-right">
                        <button type="button" onClick={() => copyShareable(g)}
                          className="rounded-lg border border-line bg-canvas px-2.5 py-1.5 text-[12px] font-semibold text-ink-2 hover:bg-subtle">
                          {copied === g.id ? "Copied ✓" : g.mode === "link" ? "Copy link" : "Copy code"}
                        </button>
                        {!g.revoked_at && (
                          <button type="button" onClick={() => revoke(g.id)}
                            className="ml-2 rounded-lg border border-line bg-canvas px-2.5 py-1.5 text-[12px] font-semibold text-deny-ink hover:bg-subtle">
                            Revoke
                          </button>
                        )}
                      </td>
