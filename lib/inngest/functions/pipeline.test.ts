...
const { stageResolveAttempt, stageSetRunning, stageTrack0, stageResolveIdentity, stagePersistIdentity, stageFindingTrack, stageSynthesis, stageVerdict, stageMemoryWrite, stageFinalize } = vi.hoisted(() => ({
  ...
  stageFindingTrack: vi.fn((_ctx: unknown, n: number) => Promise.resolve({ output: { track_key: `track_${n}` }, signal: "infer", acquisition_failed: false, failed: false, not_implemented: false, track_number: n })),
  stageSynthesis: vi.fn().mockResolvedValue({ synthesis: {} }),
  stageVerdict: vi.fn().mockReturnValue({ verdict: "verify_before_purchase", confidence_0_15: 8, certification_audits: [] }),
