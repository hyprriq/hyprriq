...
export function buildCallCPrompt(
  ...
  const system = [
    ...
    ...(advisoryContext && advisoryContext.marketplace_history.length > 0 ? [
      ...
      "brand enforcement or any other single cause, in any field. THIS INCLUDES what_to_monitor (founder-extended",
      "2026-09-11): a monitor entry states the count, its movement, and what to watch — NEVER what the shape 'may",
      "indicate', 'suggests', or is 'consistent with'. A hedge does not soften the attribution; it names one cause",
      "and stays silent about the others, which reads as our leading hypothesis. 'Currently at one against a peak of",
      "seven — any further reduction is a material signal to track' is complete; the speculation adds nothing a",
      "client can act on.",
    ] : []),
