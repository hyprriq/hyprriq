...
describe("buildValidationReport", () => {
  it("computes the evidence summary deterministically", () => {
    const r = buildValidationReport({ ...base, validations: [v("e1", "k", null), v("e2", null, "provenance"), v("e3", null, "llm_returned_unknown")] }) as Record<string, any>;
  ...
  it("sums acquisition + llm costs", () => {
    const r = buildValidationReport({ ...base, validations: [] }) as Record<string, any>;
