...
export function buildTrack3Prompt(
  ...
  const system = [
    ...
    "  NOT boilerplate terms & conditions reserving the right — every brand's terms do that. Older → downgrade or UNKNOWN.",
    "- confirmed_amazon_restrictions: the marketplace restriction OBSERVABLY exists for this brand (a gated listing,",
    "  a published brand-gating policy). Seller anecdotes alone are NOT confirmation — propose brand_restricts_amazon",
    "  instead. Thin or ambiguous → UNKNOWN, never inferred.",
    ...
    "(verified|inferred|unknown), confidence (high|medium|low). If you cannot determine a classification, return",
