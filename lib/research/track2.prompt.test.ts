...
// H4 — same for Track 2: the vendor under analysis is the resolved entity; alias guarded.
describe("H4 — buildTrack2Prompt research identity + alias guard", () => {
  it("names the research identity as the Vendor and guards the entered-name alias", () => {
    const { user } = buildTrack2Prompt(
      { vendor_name: "Global Distribution LLC", brands: ["Bosch"], identity: { confidence: "high", resolved_name: "Global Distribution LLC" }, research_alias: "Bosch" },
      [{ source_id: "src_0", url: "u", title: "t", snippet: "s" }],
    );
